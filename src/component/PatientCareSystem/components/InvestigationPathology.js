...
import { getPathologyInvestigations } from "./services/ETUCaseSheetService";

const InvestigationPathology = (props) => {
  const { pathologyDetails, setPathologyDetails } = props;
  const {
    setValue,
    watch,
    control,
  ...
  const handleClickChip = (chip) => {
    if (chip?.id === selectedChip) {
      setSelectedChip(null);
      setValue("pInvestigation", null);
    } else {
      setSelectedChip(chip.id);
      setValue("pInvestigation", chip);
      clearErrors("pInvestigation");
    }
  };
  const [pInvestigation, pIsUrgent] = watch(["pInvestigation", "pIsUrgent"]);

  const defaultValue = () => {
    setValue("pInvestigation", null);
    setValue("pIsUrgent", false);
    setIsEditMode(false);
    setSelectedChip(null);
  };
  ...
  const handleAdd = async () => {
    const isValid = await trigger("pInvestigation");
    if (pInvestigation !== null) {
      if (isValid) {
        setRecord([
          ...
          {
            id: selectedChip,
            Investigation: capitalizeStatement(pInvestigation?.label),
            "Is Urgent": pIsUrgent,
          },
        ]);
        defaultValue();
      ...
      setError("pInvestigation", { type: "custom", message: "requied" });
    }
  };
  ...
  const handleUpdate = async () => {
    const isValid = await trigger("pInvestigation");
    if (isValid) {
      const newAllergyValue = capitalizeStatement(pInvestigation.label);
  ...
  const handleEdit = (data, index) => {
    setIndex(index);
    setIsEditMode(true);
    setValue("pInvestigation", {
      id: data?.id,
      value: data?.id,
      label: data["Investigation"],
    });
    setSelectedChip(data?.id);
    setValue("pIsUrgent", data["Is Urgent"]);
    clearErrors("pInvestigation");
  };
        ...
        >
          ...
          >
            <div className="grid grid-cols-2 items-center  mb-3">
              <div className="flex font-bold justify-start">Pathology</div>
